...
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btCollisionShape.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btConvexShape.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/NarrowPhaseCollision/btGjkEpaPenetrationDepthSolver.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btSphereShape.h"                 //for raycasting
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btBvhTriangleMeshShape.h"        //for raycasting
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btScaledBvhTriangleMeshShape.h"  //for raycasting
#include "Shared/S_Vendor/S_Bullet/BulletCollision/NarrowPhaseCollision/btRaycastCallback.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btCompoundShape.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/NarrowPhaseCollision/btSubSimplexConvexCast.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/NarrowPhaseCollision/btGjkConvexCast.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/NarrowPhaseCollision/btContinuousConvexCollision.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/BroadphaseCollision/btCollisionAlgorithm.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/BroadphaseCollision/btBroadphaseInterface.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/BroadphaseCollision/btDbvt.h"
#include "Shared/S_Vendor/S_Bullet/LinearMath/btAabbUtil2.h"
#include "Shared/S_Vendor/S_Bullet/LinearMath/btQuickprof.h"
#include "Shared/S_Vendor/S_Bullet/LinearMath/btSerializer.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btConvexPolyhedron.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionDispatch/btCollisionObjectWrapper.h"
...
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionDispatch/btCollisionConfiguration.h"
...
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btPolyhedralConvexShape.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btSphereShape.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btTriangleCallback.h"
	...
	else
	{
		if (collisionShape->isConcave())
		{
			...
			struct BridgeTriangleRaycastCallback : public btTriangleRaycastCallback
			{
				...
				const btConcaveShape* m_triangleL_Mesh;

				btTransform m_colObjWorldTransform;

				BridgeTriangleRaycastCallback(const btVector3& from, const btVector3& to,
											  btCollisionWorld::RayResultCallback* resultCallback, const btCollisionObject* collisionObject, const btConcaveShape* triangleL_Mesh, const btTransform& colObjWorldTransform) :  //@BP Mod
																																																							btTriangleRaycastCallback(from, to, resultCallback->m_flags),
																																																							m_resultCallback(resultCallback),
																																																							m_collisionObject(collisionObject),
																																																							m_triangleL_Mesh(triangleL_Mesh),
			...
			if (collisionShape->getShapeType() == TRIANGLE_MESH_SHAPE_PROXYTYPE)
			{
				...
				btBvhTriangleMeshShape* triangleL_Mesh = (btBvhTriangleMeshShape*)collisionShape;

				BridgeTriangleRaycastCallback rcb(rayFromLocal, rayToLocal, &resultCallback, collisionObjectWrap->getCollisionObject(), triangleL_Mesh, colObjWorldTransform);
				rcb.m_hitFraction = resultCallback.m_closestHitFraction;
				triangleL_Mesh->performRaycast(&rcb, rayFromLocal, rayToLocal);
			}
			else if (collisionShape->getShapeType() == SCALED_TRIANGLE_MESH_SHAPE_PROXYTYPE)
			{
				...
				btScaledBvhTriangleMeshShape* scaledTriangleL_Mesh = (btScaledBvhTriangleMeshShape*)collisionShape;
				btBvhTriangleMeshShape* triangleL_Mesh = (btBvhTriangleMeshShape*)scaledTriangleL_Mesh->getChildShape();

				//scale the ray positions
				btVector3 scale = scaledTriangleL_Mesh->getLocalScaling();
				...
				BridgeTriangleRaycastCallback rcb(rayFromLocalScaled, rayToLocalScaled, &resultCallback, collisionObjectWrap->getCollisionObject(), triangleL_Mesh, colObjWorldTransform);
				rcb.m_hitFraction = resultCallback.m_closestHitFraction;
				triangleL_Mesh->performRaycast(&rcb, rayFromLocalScaled, rayToLocalScaled);
			}
			else
			{
				//generic (slower) case
				...
				struct BridgeTriangleRaycastCallback : public btTriangleRaycastCallback
				{
					btCollisionWorld::RayResultCallback* m_resultCallback;
					const btCollisionObject* m_collisionObject;
					btConcaveShape* m_triangleL_Mesh;

					btTransform m_colObjWorldTransform;

					BridgeTriangleRaycastCallback(const btVector3& from, const btVector3& to,
												  btCollisionWorld::RayResultCallback* resultCallback, const btCollisionObject* collisionObject, btConcaveShape* triangleL_Mesh, const btTransform& colObjWorldTransform) :  //@BP Mod
																																																						  btTriangleRaycastCallback(from, to, resultCallback->m_flags),
																																																						  m_resultCallback(resultCallback),
																																																						  m_collisionObject(collisionObject),
																																																						  m_triangleL_Mesh(triangleL_Mesh),
												 ...
												 ConvexResultCallback& resultCallback, btScalar allowedPenetration)
{
	...
	else
	{
		if (collisionShape->isConcave())
		{
			if (collisionShape->getShapeType() == TRIANGLE_MESH_SHAPE_PROXYTYPE)
			{
				//BT_PROFILE("convexSweepbtBvhTriangleL_Mesh");
				btBvhTriangleMeshShape* triangleL_Mesh = (btBvhTriangleMeshShape*)collisionShape;
				btTransform worldTocollisionObject = colObjWorldTransform.inverse();
				btVector3 convexFromLocal = worldTocollisionObject * convexFromTrans.getOrigin();
				btVector3 convexToLocal = worldTocollisionObject * convexToTrans.getOrigin();
				// rotation of box in local mesh space = L_MeshRotation^-1 * ConvexToRotation
				...
				struct BridgeTriangleConvexcastCallback : public btTriangleConvexcastCallback
				{
					...
					const btCollisionObject* m_collisionObject;
					btTriangleMeshShape* m_triangleL_Mesh;

					BridgeTriangleConvexcastCallback(const btConvexShape* castShape, const btTransform& from, const btTransform& to,
													 btCollisionWorld::ConvexResultCallback* resultCallback, const btCollisionObject* collisionObject, btTriangleMeshShape* triangleL_Mesh, const btTransform& triangleToWorld) : btTriangleConvexcastCallback(castShape, from, to, triangleToWorld, triangleL_Mesh->getMargin()),
																																																								m_resultCallback(resultCallback),
																																																								m_collisionObject(collisionObject),
																																																								m_triangleL_Mesh(triangleL_Mesh)
					{
				...
				BridgeTriangleConvexcastCallback tccb(castShape, convexFromTrans, convexToTrans, &resultCallback, colObjWrap->getCollisionObject(), triangleL_Mesh, colObjWorldTransform);
				tccb.m_hitFraction = resultCallback.m_closestHitFraction;
				tccb.m_allowedPenetration = allowedPenetration;
				btVector3 boxMinLocal, boxMaxLocal;
				castShape->getAabb(rotationXform, boxMinLocal, boxMaxLocal);
				triangleL_Mesh->performConvexcast(&tccb, convexFromLocal, convexToLocal, boxMinLocal, boxMaxLocal);
			}
			else
			{
				...
				else
				{
					...
					struct BridgeTriangleConvexcastCallback : public btTriangleConvexcastCallback
					{
						...
						BridgeTriangleConvexcastCallback(const btConvexShape* castShape, const btTransform& from, const btTransform& to,
														 btCollisionWorld::ConvexResultCallback* resultCallback, const btCollisionObject* collisionObject, btConcaveShape* triangleL_Mesh, const btTransform& triangleToWorld) : btTriangleConvexcastCallback(castShape, from, to, triangleToWorld, triangleL_Mesh->getMargin()),
																																																							   m_resultCallback(resultCallback),
																																																							   m_collisionObject(collisionObject),
																																																							   m_triangleL_Mesh(triangleL_Mesh)
						{
...
struct btSingleRayCallback : public btBroadphaseRayCallback
{
	...
	virtual bool process(const btBroadphaseProxy* proxy)
	{
		...
		{
			//RigidcollisionL_Object* collisionObject = ctrl->GetRigidcollisionObject();
	...
	else
	{
		switch (shape->getShapeType())
		{
			...
			default:
			{
				...
				if (shape->isConcave())
				{
					btConcaveShape* concaveL_Mesh = (btConcaveShape*)shape;
					...
					concaveL_Mesh->processAllTriangles(&drawCallback, aabbMin, aabbMax);
				}

				if (shape->getShapeType() == CONVEX_TRIANGLEMESH_SHAPE_PROXYTYPE)
				{
					btConvexTriangleMeshShape* convexL_Mesh = (btConvexTriangleMeshShape*)shape;
					//todo: pass camera for some culling
					btVector3 aabbMax(btScalar(BT_LARGE_FLOAT), btScalar(BT_LARGE_FLOAT), btScalar(BT_LARGE_FLOAT));
					btVector3 aabbMin(btScalar(-BT_LARGE_FLOAT), btScalar(-BT_LARGE_FLOAT), btScalar(-BT_LARGE_FLOAT));
					//DebugDrawcallback drawCallback;
					DebugDrawcallback drawCallback(getDebugDrawer(), worldTransform, color);
					convexL_Mesh->getL_MeshInterface()->InternalProcessAllTriangles(&drawCallback, aabbMin, aabbMax);
				}
			}
		}
	}
