...
#include "Shared/S_Vendor/S_Bullet/LinearMath/btSerializer.h"  //for btBulletSerializedArrays definition
...
void btCollisionWorldImporter::deleteAllData()
{
	...
	for (i = 0; i < m_allocatedbtStridingMeshInterfaceDatas.size(); i++)
	{
		...
		for (int a = 0; a < curData->m_numL_MeshParts; a++)
		{
			btL_MeshPartData* curPart = &curData->m_meshPartsPtr[a];
...
btCollisionShape* btCollisionWorldImporter::convertCollisionShape(btCollisionShapeData* shapeData)
{
	...
	switch (shapeData->m_shapeType)
	{
		...
		{
			btScaledTriangleL_MeshShapeData* scaledL_Mesh = (btScaledTriangleL_MeshShapeData*)shapeData;
			btCollisionShapeData* colShapeData = (btCollisionShapeData*)&scaledL_Mesh->m_trimeshShapeData;
			...
			localScaling.deSerializeFloat(scaledL_Mesh->m_localScaling);

			shape = createScaledTrangleL_MeshShape(meshShape, localScaling);
			break;
		}
#ifdef SUPPORT_GIMPACT_SHAPE_IMPORT
		case GIMPACT_SHAPE_PROXYTYPE:
		{
			btGImpactL_MeshShapeData* gimpactData = (btGImpactL_MeshShapeData*)shapeData;
			if (gimpactData->m_gimpactSubType == CONST_GIMPACT_TRIMESH_SHAPE)
			{
				btStridingMeshInterfaceData* interfaceData = createStridingL_MeshInterfaceData(&gimpactData->m_meshInterface);
				btTriangleIndexVertexArray* meshInterface = createL_MeshInterface(*interfaceData);

				btGImpactL_MeshShape* gimpactShape = createGimpactShape(meshInterface);
				btVector3 localScaling;
		...
		case TRIANGLE_MESH_SHAPE_PROXYTYPE:
		{
			...
			btStridingMeshInterfaceData* interfaceData = createStridingL_MeshInterfaceData(&trimesh->m_meshInterface);
			btTriangleIndexVertexArray* meshInterface = createL_MeshInterface(*interfaceData);
			...
			btBvhTriangleMeshShape* trimeshShape = createBvhTriangleL_MeshShape(meshInterface, bvh);
...
btTriangleIndexVertexArray* btCollisionWorldImporter::createL_MeshInterface(btStridingMeshInterfaceData& meshData)
{
	btTriangleIndexVertexArray* meshInterface = createTriangleL_MeshContainer();

	for (int i = 0; i < meshData.m_numL_MeshParts; i++)
	{
		btIndexedL_Mesh meshPart;
		...
		{
			meshInterface->addIndexedL_Mesh(meshPart, meshPart.m_indexType);
		}
	}
...
btStridingMeshInterfaceData* btCollisionWorldImporter::createStridingL_MeshInterfaceData(btStridingMeshInterfaceData* interfaceData)
{
	...
	newData->m_numL_MeshParts = interfaceData->m_numL_MeshParts;
	newData->m_meshPartsPtr = new btL_MeshPartData[newData->m_numL_MeshParts];

	for (int i = 0; i < newData->m_numL_MeshParts; i++)
	{
		btL_MeshPartData* curPart = &interfaceData->m_meshPartsPtr[i];
		btL_MeshPartData* curNewPart = &newData->m_meshPartsPtr[i];
...
btTriangleIndexVertexArray* btCollisionWorldImporter::createTriangleL_MeshContainer()
{
...
btBvhTriangleMeshShape* btCollisionWorldImporter::createBvhTriangleL_MeshShape(btStridingMeshInterface* trimesh, btOptimizedBvh* bvh)
{
	if (bvh)
	{
		btBvhTriangleMeshShape* bvhTriL_Mesh = new btBvhTriangleMeshShape(trimesh, bvh->isQuantized(), false);
		bvhTriL_Mesh->setOptimizedBvh(bvh);
		m_allocatedCollisionShapes.push_back(bvhTriL_Mesh);
		return bvhTriL_Mesh;
	}
...
btCollisionShape* btCollisionWorldImporter::createConvexTriangleL_MeshShape(btStridingMeshInterface* trimesh)
{
	return 0;
}
#ifdef SUPPORT_GIMPACT_SHAPE_IMPORT
btGImpactL_MeshShape* btCollisionWorldImporter::createGimpactShape(btStridingMeshInterface* trimesh)
{
	btGImpactL_MeshShape* shape = new btGImpactL_MeshShape(trimesh);
	m_allocatedCollisionShapes.push_back(shape);
	return shape;
}
#endif  //SUPPORT_GIMPACT_SHAPE_IMPORT
...
btScaledBvhTriangleMeshShape* btCollisionWorldImporter::createScaledTrangleL_MeshShape(btBvhTriangleMeshShape* meshShape, const btVector3& localScaling)
{
