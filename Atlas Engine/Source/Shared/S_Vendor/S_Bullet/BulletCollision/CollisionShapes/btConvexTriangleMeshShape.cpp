...
btConvexTriangleMeshShape ::btConvexTriangleMeshShape(btStridingMeshInterface* meshInterface, bool calcAabb)
	: btPolyhedralConvexAabbCachingShape(), m_stridingL_Mesh(meshInterface)
{
...
btVector3 btConvexTriangleMeshShape::localGetSupportingVertexWithoutMargin(const btVector3& vec0) const
{
	...
	m_stridingL_Mesh->InternalProcessAllTriangles(&supportCallback, -aabbMax, aabbMax);
...
{
	m_stridingL_Mesh->setScaling(scaling);
...
{
	return m_stridingL_Mesh->getScaling();
}
...
void btConvexTriangleMeshShape::calculatePrincipalAxisTransform(btTransform& principal, btVector3& inertia, btScalar& volume) const
{
	...
	btVector3 aabbMax(btScalar(BT_LARGE_FLOAT), btScalar(BT_LARGE_FLOAT), btScalar(BT_LARGE_FLOAT));
	m_stridingL_Mesh->InternalProcessAllTriangles(&centerCallback, -aabbMax, aabbMax);
	btVector3 center = centerCallback.getCenter();
	principal.setOrigin(center);
	volume = centerCallback.getVolume();

	InertiaCallback inertiaCallback(center);
	m_stridingL_Mesh->InternalProcessAllTriangles(&inertiaCallback, -aabbMax, aabbMax);
