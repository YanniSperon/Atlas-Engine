...
const char* btStridingMeshInterface::serialize(void* dataBuffer, btSerializer* serializer) const
{
	...
	trimeshData->m_numL_MeshParts = getNumSubParts();

	//void* uniquePtr = 0;

	trimeshData->m_meshPartsPtr = 0;

	if (trimeshData->m_numL_MeshParts)
	{
		btChunk* chunk = serializer->allocate(sizeof(btL_MeshPartData), trimeshData->m_numL_MeshParts);
		btL_MeshPartData* memPtr = (btL_MeshPartData*)chunk->m_oldPtr;
		trimeshData->m_meshPartsPtr = (btL_MeshPartData*)serializer->getUniquePointer(memPtr);
		...
		serializer->finalizeChunk(chunk, "btL_MeshPartData", BT_ARRAY_CODE, chunk->m_oldPtr);
	}
