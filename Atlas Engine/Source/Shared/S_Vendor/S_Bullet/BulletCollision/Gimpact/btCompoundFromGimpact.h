...
{
	const btGImpactL_MeshShape* m_gimpactShape;
	btCompoundShape* m_colShape;
	btScalar m_depth;

	MyInternalTriangleIndexCallback(btCompoundShape* colShape, const btGImpactL_MeshShape* meshShape, btScalar depth)
		: m_colShape(colShape),
...
btCompoundShape* btCreateCompoundFromGimpactShape(const btGImpactL_MeshShape* gimpactL_Mesh, btScalar depth)
{
	...
	MyInternalTriangleIndexCallback cb(colShape, gimpactL_Mesh, depth);
	btVector3 aabbMin, aabbMax;
	gimpactL_Mesh->getAabb(tr, aabbMin, aabbMax);
	gimpactL_Mesh->getL_MeshInterface()->InternalProcessAllTriangles(&cb, aabbMin, aabbMax);
