...
void btGImpactCollisionAlgorithm::collide_gjk_triangles(const btCollisionObjectWrapper* body0Wrap,
														const btCollisionObjectWrapper* body1Wrap,
														const btGImpactL_MeshShapePart* shape0,
														const btGImpactL_MeshShapePart* shape1,
	...
	const btGImpactShapeInterface* shape1)
{
	...
	{
		const btGImpactL_MeshShape* meshshape0 = static_cast<const btGImpactL_MeshShape*>(shape0);
		m_part0 = meshshape0->getL_MeshPartCount();

		while (m_part0--)
		{
			gimpact_vs_gimpact(body0Wrap, body1Wrap, meshshape0->getL_MeshPart(m_part0), shape1);
		}
	...
	{
		const btGImpactL_MeshShape* meshshape1 = static_cast<const btGImpactL_MeshShape*>(shape1);
		m_part1 = meshshape1->getL_MeshPartCount();

		while (m_part1--)
		{
			gimpact_vs_gimpact(body0Wrap, body1Wrap, shape0, meshshape1->getL_MeshPart(m_part1));
		}
	...
	{
		const btGImpactL_MeshShapePart* shapepart0 = static_cast<const btGImpactL_MeshShapePart*>(shape0);
		const btGImpactL_MeshShapePart* shapepart1 = static_cast<const btGImpactL_MeshShapePart*>(shape1);
												   ...
												   const btCollisionShape* shape1, bool swapped)
{
	if (shape0->getGImpactShapeType() == CONST_GIMPACT_TRIMESH_SHAPE)
	{
		...
		part = meshshape0->getL_MeshPartCount();

		while (part--)
		{
			gimpact_vs_shape(body0Wrap,
							 body1Wrap,
							 meshshape0->getL_MeshPart(part),
							 shape1, swapped);
		}
	...
	{
		const btGImpactL_MeshShapePart* shapepart = static_cast<const btGImpactL_MeshShapePart*>(shape0);
