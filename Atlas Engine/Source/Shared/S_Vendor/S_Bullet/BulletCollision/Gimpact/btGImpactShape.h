...
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btTriangleShape.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btStridingMeshInterface.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btCollisionMargin.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionDispatch/btCollisionWorld.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btConcaveShape.h"
#include "Shared/S_Vendor/S_Bullet/BulletCollision/CollisionShapes/btTetrahedronShape.h"
#include "Shared/S_Vendor/S_Bullet/LinearMath/btVector3.h"
#include "Shared/S_Vendor/S_Bullet/LinearMath/btTransform.h"
#include "Shared/S_Vendor/S_Bullet/LinearMath/btMatrix3x3.h"
...
/*!
- Simply create this shape by passing the btStridingMeshInterface to the constructor btGImpactL_MeshShapePart, then you must call updateBound() after creating the mesh
...
*/
class btGImpactL_MeshShapePart : public btGImpactShapeInterface
{
public:
...
public:
	btGImpactL_MeshShapePart()
	{
		m_box_set.setPrimitiveManager(&m_primitive_manager);
	}

	btGImpactL_MeshShapePart(btStridingMeshInterface* meshInterface, int part);
	virtual ~btGImpactL_MeshShapePart();
...
/*!
Set of btGImpactL_MeshShapePart parts
- Simply create this shape by passing the btStridingMeshInterface to the constructor btGImpactL_MeshShape, then you must call updateBound() after creating the mesh

- You can handle deformable meshes with this shape, by calling postUpdate() every time when changing the mesh vertices.

*/
class btGImpactL_MeshShape : public btGImpactShapeInterface
{
	btStridingMeshInterface* m_meshInterface;

protected:
	btAlignedObjectArray<btGImpactL_MeshShapePart*> m_mesh_parts;
	void buildL_MeshParts(btStridingMeshInterface* meshInterface)
	{
		for (int i = 0; i < meshInterface->getNumSubParts(); ++i)
		{
			btGImpactL_MeshShapePart* newpart = new btGImpactL_MeshShapePart(meshInterface, i);
...
public:
	btGImpactL_MeshShape(btStridingMeshInterface* meshInterface)
	{
		m_meshInterface = meshInterface;
		buildL_MeshParts(meshInterface);
	}

	virtual ~btGImpactL_MeshShape()
	{
		int i = m_mesh_parts.size();
		while (i--)
		{
			btGImpactL_MeshShapePart* part = m_mesh_parts[i];
			delete part;
		}
		m_mesh_parts.clear();
	}

	btStridingMeshInterface* getL_MeshInterface()
	{
		return m_meshInterface;
	}

	const btStridingMeshInterface* getL_MeshInterface() const
	{
		return m_meshInterface;
	}

	int getL_MeshPartCount() const
	{
		return m_mesh_parts.size();
	}

	btGImpactL_MeshShapePart* getL_MeshPart(int index)
	{
		return m_mesh_parts[index];
	}

	const btGImpactL_MeshShapePart* getL_MeshPart(int index) const
	{
		return m_mesh_parts[index];
	}
...
struct btGImpactL_MeshShapeData
{
...
SIMD_FORCE_INLINE int btGImpactL_MeshShape::calculateSerializeBufferSize() const
{
	return sizeof(btGImpactL_MeshShapeData);
}
